...
| `src/app.ts` | `mountApp(root: HTMLElement): void`。フォーム、画面状態、キャッシュと通信の連携、エラー表示、検索・フィルタ・並び替え、カード描画、`gh-repo-viewer` のみのフッター表示を担当する。 |
...
| `src/style.css` | 既存の色、カード、フォーム、レスポンシブレイアウト、フッターの小さな文字（11px）と中央揃え。公開関数なし。 |
...
| `tsconfig.json` | TypeScript の型検査設定。 |
| `package.json` | 開発、ビルド、プレビュー、テストのコマンドと開発依存。 |
